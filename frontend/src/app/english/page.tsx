...
import { ArrowLeft, Search, Loader2, BookOpen, Quote, PenTool, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify , LayoutTemplate} from "lucide-react";
...
export default function EnglishTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("english_track_company") || "");
        setTopic(sessionStorage.getItem("english_track_topic") as any || "Grammar");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("english_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/english/generate", {
        method: "POST",
      ...
      sessionStorage.setItem("english_track", JSON.stringify(problemData));
      sessionStorage.setItem("english_track_id", Date.now().toString());
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-indigo-500/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <BookOpen className="h-7 w-7 text-indigo-500" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">English Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">Curated grammar and verbal tracks for your target company</p>
          </div>
        </div>
          ...
          questions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              <div className="bg-card border border-border p-4 rounded-2xl mb-6 shadow-sm">
                <Quote className="h-8 w-8 text-indigo-500" strokeWidth={1.5} />
              </div>
              <h2 className="text-[22px] font-bold text-foreground mb-2">No English tests assigned</h2>
              ...
              {filteredQuestions.map((q, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <Button size="icon" className="bg-indigo-600 hover:bg-orange-700 text-white rounded-full h-11 w-11 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("english_track");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_english_problem", JSON.stringify(sub.problemData));
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-pink-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "English Test"}</h3>
