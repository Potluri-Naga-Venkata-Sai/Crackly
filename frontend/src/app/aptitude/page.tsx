...
import { ArrowLeft, Search, Loader2, LineChart, Brain, MessageCircle, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify , LayoutTemplate} from "lucide-react";
...
export default function AptitudeTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("aptitude_track_company") || "");
        setTopic(sessionStorage.getItem("aptitude_track_topic") as any || "Quantitative");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("aptitude_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/aptitude/generate", {
        method: "POST",
      ...
      sessionStorage.setItem("aptitude_track", JSON.stringify(problemData));
      sessionStorage.setItem("aptitude_track_id", Date.now().toString());
  ...
  const handleStartPractice = (problem: any) => {
    sessionStorage.setItem("current_aptitude_problem", JSON.stringify(problem));
    router.push("/aptitude/practice");
  };
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-orange-500/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <LineChart className="h-7 w-7 text-orange-500" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">Aptitude Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">Curated reasoning & aptitude tracks for your target company</p>
          </div>
        </div>
          ...
          questions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              <div className="bg-card border border-border p-4 rounded-2xl mb-6 shadow-sm">
                <Brain className="h-8 w-8 text-orange-500" strokeWidth={1.5} />
              </div>
              <h2 className="text-[22px] font-bold text-foreground mb-2">No Aptitude tests assigned</h2>
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("aptitude_track");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_aptitude_problem", JSON.stringify(sub.problemData));
                    router.push("/aptitude/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-emerald-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "Aptitude Test"}</h3>
