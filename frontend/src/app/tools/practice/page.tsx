...
export default function ToolsPracticePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const submissions = JSON.parse(localStorage.getItem("tools_submissions") || "[]");
      const newSubmission = {
      ...
      await saveSubmissionLocallyAndToCloud("tools", newSubmission);
      ...
      logActivity({
        ...
        checkTrackCompletion("tools", trackQuestions, [newSubmission, ...submissions]);
    } catch (err) {
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-4 h-full min-h-0">
          ...
          {result && (
            <div className="p-6 bg-card/50 flex-1">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Ideal STAR Structure</h4>
        ...
        <div className="bg-background border border-border/50 rounded-xl flex flex-col min-h-0 relative">
          <div className="p-4 border-b border-border/50 bg-card rounded-t-xl flex justify-between items-center">
            <span className="text-sm font-bold text-muted-foreground">Your Answer (Use STAR Method)</span>
            <Button 
              size="sm" 
