...
import { ArrowLeft, Search, Loader2, ListFilter, Flame, Server, Code2, Database, UploadCloud, FileText, CheckCircle2, Play, Clock, ChevronRight, Menu, ChevronDown, AlignJustify , LayoutTemplate} from "lucide-react";
...
export default function MCQTestsPage() {
  ...
  useEffect(() => {
    const track = sessionStorage.getItem("mcq_track");
    if (track) {
      try {
    ...
    const savedSubmissions = localStorage.getItem("mcq_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerateStandard = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/mcq/generate", {
        method: "POST",
      ...
      sessionStorage.setItem("mcq_track", JSON.stringify(problemData));
      sessionStorage.setItem("mcq_track_id", Date.now().toString());
      localStorage.setItem("mcq_generations", (parseInt(localStorage.getItem("mcq_generations") || "0") + 1).toString());
    } catch (err: any) {
      setError(err.message || "Failed to generate problems.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateResume = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/mcq/generate-from-resume", {
        method: "POST",
        body: formData,
      });
  ...
  const handleStartPractice = (problem: any) => {
    sessionStorage.setItem("current_mcq_problem", JSON.stringify(problem));
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-purple-500/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <ListFilter className="h-7 w-7 text-purple-400" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">MCQ Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">Advanced multiple choice questions for core tech subjects</p>
          </div>
        </div>
            ...
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-8 text-center bg-muted/50">
              ...
              {mode === "standard" ? (
                <div className="w-full max-w-md bg-background border border-border p-6 rounded-2xl shadow-lg animate-in fade-in zoom-in-95 duration-200">
                  <h3 className="text-foreground font-bold mb-4 text-left">Generate Target Track</h3>
                  
                  <div className="flex gap-2 mb-4 p-1 bg-card border border-border rounded-lg">
                  ...
                  <div className="flex flex-col gap-3">
                    <Input 
                      ...
                      onKeyDown={(e) => e.key === "Enter" && handleGenerateStandard()}
                    />
                ...
                <div className="w-full max-w-md bg-background border border-border p-6 rounded-2xl shadow-lg animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <p className="text-xs text-muted-foreground text-left mb-4">Extract your tech stack to generate highly personalized MCQs.</p>
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-[17px] font-bold text-foreground group-hover:text-purple-400 transition-colors">{q.title}</h3>
                    </div>
                  ...
                  <div className="flex items-center gap-5">
                    <div className="hidden sm:block text-right">
                      <div className={cn("text-sm font-semibold", q.company === "Personalized" ? "text-purple-400" : "text-muted-foreground")}>{q.company}</div>
                      <div className="text-[11px] text-muted-foreground font-medium">{q.company === "Personalized" ? "Resume Extracted" : "Target Company"}</div>
                    </div>
                    <Button size="icon" className="bg-purple-600 hover:bg-purple-700 text-foreground rounded-full h-11 w-11 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("mcq_track");
          ...
          filteredSubmissions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground max-w-[500px] leading-relaxed">
                You haven't submitted any MCQ solutions yet. Go to the All tab, pick a problem, and submit your answer to track your history here.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_mcq_problem", JSON.stringify(sub.problemData));
                    router.push("/mcq/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-amber-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "MCQ Test"}</h3>
