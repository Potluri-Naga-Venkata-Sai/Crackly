...
import { ArrowLeft, Search, Code, Code2, Loader2, Database, Braces, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify } from "lucide-react";
...
export default function SQLTestsPage() {
  ...
  const [topic, setTopic] = useState<"SQL">("SQL");
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("sql_track_company") || "");
        setTopic("SQL");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("sql_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Persist the track
      sessionStorage.setItem("sql_track", JSON.stringify(problemData));
      sessionStorage.setItem("sql_track_id", Date.now().toString());
      ...
      sessionStorage.setItem("sql_track_topic", topic);
    } catch (err: any) {
      setError(err.message || "Failed to generate sql problems. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStartPractice = (problem: any) => {
    sessionStorage.setItem("current_sql_problem", JSON.stringify(problem));
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">SQL Tests</h1>
        ...
        {activeTab === "all" ? (
          questions.length === 0 ? (
            ...
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <h2 className="text-[22px] font-bold text-foreground mb-2">No SQL tests assigned</h2>
              ...
              <div className="w-full max-w-md bg-background border border-border p-6 rounded-2xl shadow-lg">
                <h3 className="text-foreground font-bold mb-4 text-left">Generate SQL Track</h3>
                
                {/* Removed Topic Selection for SQL Only */}
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("sql_track");
          ...
          filteredSubmissions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground max-w-[500px] leading-relaxed">
                You haven&apos;t submitted any sql solutions yet. Go to the All tab, pick a problem, and submit your code to track your history here.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_sql_problem", JSON.stringify(sub.problemData));
                    router.push("/sql/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-rose-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "SQL Test"}</h3>
