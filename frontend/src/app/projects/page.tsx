...
import { ArrowLeft, Search, Loader2, Briefcase, MessageCircle, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify } from "lucide-react";
...
export default function ProjectsTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("projects_track_company") || "");
        setTopic((sessionStorage.getItem("projects_track_topic") as any) || "General");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("projects_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const payload: any = { project_description: company };
      if (topic === "Stream") {
        payload.program = program;
      }
      
      const res = await fetch("http://localhost:8000/api/projects/generate", {
        method: "POST",
      ...
      sessionStorage.setItem("projects_track", JSON.stringify(problemData));
      sessionStorage.setItem("projects_track_id", Date.now().toString());
      localStorage.setItem("projects_generations", (parseInt(localStorage.getItem("projects_generations") || "0") + 1).toString());
      sessionStorage.setItem("projects_track_company", company);
      sessionStorage.setItem("projects_track_topic", topic);
    } catch (err: any) {
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-orange-500/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <Briefcase className="h-7 w-7 text-orange-500" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">Project Evaluation</h1>
            <p className="text-sm text-muted-foreground font-medium">AI-based project review and scoring</p>
          </div>
        </div>
          ...
          questions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              <div className="bg-card border border-border p-4 rounded-2xl mb-6 shadow-sm">
                <Briefcase className="h-8 w-8 text-orange-500" strokeWidth={1.5} />
              </div>
              <h2 className="text-[22px] font-bold text-foreground mb-2">No Project evaluations assigned</h2>
              ...
              <div className="w-full max-w-md bg-background border border-border p-6 rounded-2xl shadow-lg">
                ...
                <div className="flex flex-col gap-3">
                  <Input 
                    placeholder="Enter Project Description (e.g. built a react app...)"
                    value={company}
              ...
              {filteredQuestions.map((q, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <Button size="icon" className="bg-orange-600 hover:bg-orange-700 text-foreground rounded-full h-11 w-11 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("projects_track");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_projects_problem", JSON.stringify(sub.problemData));
                    router.push("/projects/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-orange-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "Projects Test"}</h3>
