...
import { Loader2, Briefcase, MessageCircle, Play, Flame } from "lucide-react";
...
export default function ProjectsTestsPage() {
  ...
  const handleGenerate = async (onSuccess: (data: any[]) => void, onError: (err: string) => void) => {
    ...
    try {
      const payload: any = { project_description: description };
      if (topic === "Stream") {
        payload.program = program;
      }
      
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/projects/generate`, {
        method: "POST",
      ...
      sessionStorage.setItem("projects_track_company", description); // legacy key names kept for compatibility
  ...
  return (
    <ModuleLayout
      ...
      emptyStateTitle="No Project evaluations assigned"
      ...
      getQuestionRoute={() => "/projects/practice"}
