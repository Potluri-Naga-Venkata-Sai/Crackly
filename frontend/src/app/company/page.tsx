...
export default function CompanyTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("company_track_company") || "");
        setTopic((sessionStorage.getItem("company_track_topic") as any) || "General");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("company_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/api/company/generate", {
        method: "POST",
      ...
      sessionStorage.setItem("company_track", JSON.stringify(problemData));
      sessionStorage.setItem("company_track_id", Date.now().toString());
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">Company Fit Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">Behavioral and leadership principles</p>
          </div>
        </div>
        ...
        {activeTab === "all" ? (
          questions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <h2 className="text-[22px] font-bold text-foreground mb-2">No Company Fit tests assigned</h2>
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("company_track");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_company_problem", JSON.stringify(sub.problemData));
                    router.push("/company/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-violet-500/10 text-violet-500 flex items-center justify-center shrink-0 border border-violet-500/20">
                        <Building2 className="w-5 h-5" />
                      </div>
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-violet-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "Company Fit Test"}</h3>
