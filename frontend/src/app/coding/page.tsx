...
import { ArrowLeft, Search, Code, Code2, Loader2, Database, Braces, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify, LayoutTemplate } from "lucide-react";
...
export default function CodingTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("coding_track_company") || "");
        setTopic(sessionStorage.getItem("coding_track_topic") as "DSA" | "SQL" | "Stream" || "DSA");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("coding_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:8000/api/dsa/generate", {
        method: "POST",
      ...
      // Persist the track
      sessionStorage.setItem("coding_track", JSON.stringify(problemData));
      sessionStorage.setItem("coding_track_id", Date.now().toString());
      ...
      setError(err.message || "Failed to generate coding problems. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStartPractice = (problem: any) => {
    sessionStorage.setItem("current_coding_problem", JSON.stringify(problem));
  ...
  // Pending questions are questions in the current track that haven't been submitted yet.
  const pendingCount = questions.filter(q => !submissions.some(s => s.problemTitle === q.title)).length;
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-blue-500/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <Code2 className="h-7 w-7 text-blue-400" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">Coding Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">DSA practice and code-based tasks</p>
          </div>
        </div>
          ...
          questions.length === 0 ? (
            ...
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              <div className="bg-card border border-border p-4 rounded-2xl mb-6 shadow-sm">
                <Code className="h-8 w-8 text-muted-foreground" strokeWidth={1.5} />
              </div>
              <h2 className="text-[22px] font-bold text-foreground mb-2">No Coding tests assigned</h2>
            ...
            <div className="grid grid-cols-1 gap-3">
                ...
                >
                  <div className="flex flex-col gap-1.5">
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-[17px] font-bold text-foreground group-hover:text-blue-400 transition-colors">{q.title}</h3>
                    </div>
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <Button size="icon" className="bg-blue-600 hover:bg-blue-700 text-foreground rounded-full h-11 w-11 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
              ...
              {/* Optional footer action to clear track */}
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("coding_track");
          ...
          filteredSubmissions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground max-w-[500px] leading-relaxed">
                You haven't submitted any coding solutions yet. Go to the All tab, pick a problem, and submit your code to track your history here.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_coding_problem", JSON.stringify(sub.problemData));
                    router.push("/coding/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-blue-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "Coding Test"}</h3>
