...
export default function SystemTestsPage() {
  ...
  const handleGenerate = async (onSuccess: (data: any[]) => void, onError: (err: string) => void) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/system/generate`, {
        method: "POST",
  ...
  return (
    <ModuleLayout
      moduleKey="system"
      title="System Design Tests"
      description="HLD and LLD architecture practice"
      icon={<Layers className="h-7 w-7 text-emerald-500" strokeWidth={2} />}
      iconColorClass="text-emerald-500"
      iconBgClass="bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
      emptyStateIcon={<Layers className="h-8 w-8 text-emerald-500" strokeWidth={1.5} />}
      emptyStateTitle="No System Design tests assigned"
      ...
      getQuestionRoute={() => "/system/practice"}
      renderQuestionMetadata={(q) => (
        <span className="text-xs text-muted-foreground flex items-center gap-1.5">
          <Layers className="w-3.5 h-3.5" />
          {q.topic || "System Design"}
        </span>
      )}
