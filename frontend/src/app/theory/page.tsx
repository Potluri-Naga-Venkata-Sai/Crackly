...
import { ArrowLeft, Search, Loader2, BookOpen, Brain, MessageCircle, Play, Flame, Clock, ChevronRight, Menu, CheckCircle2, ChevronDown, AlignJustify , LayoutTemplate} from "lucide-react";
...
export default function TheoryTestsPage() {
  ...
  useEffect(() => {
    ...
    if (track) {
      try {
        ...
        setCompany(sessionStorage.getItem("theory_track_company") || "");
        setTopic(sessionStorage.getItem("theory_track_topic") as any || "Computer Science");
      } catch(e) {}
    }

    const savedSubmissions = localStorage.getItem("theory_submissions");
    if (savedSubmissions) {
      try {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      sessionStorage.setItem("theory_track", JSON.stringify(problemData));
      sessionStorage.setItem("theory_track_id", Date.now().toString());
    ...
    <div className="flex-1 p-8 font-sans max-w-[1400px] mx-auto w-full h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="bg-muted border border-zinc-400/20 p-3 rounded-xl flex items-center justify-center shadow-inner">
            <BookOpen className="h-7 w-7 text-muted-foreground" strokeWidth={2} />
          </div>
          <div>
            <h1 className="text-[28px] font-bold text-foreground tracking-tight leading-tight mb-1">Theory Tests</h1>
            <p className="text-sm text-muted-foreground font-medium">Computer Science fundamentals and core theory concepts</p>
          </div>
        </div>
        ...
        {activeTab === "all" ? (
          questions.length === 0 ? (
            <div className="flex-1 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center p-12 text-center bg-muted/50">
              ...
              <h2 className="text-[22px] font-bold text-foreground mb-2">No Theory tests assigned</h2>
                ...
                <div className="flex gap-2 mb-4 p-1 bg-card border border-border rounded-lg">
                  <button
                    ...
                    className={cn(
                      ...
                      topic === "Computer Science" ? "bg-zinc-600 text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
              ...
              {filteredQuestions.map((q, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <Button size="icon" className="bg-zinc-600 hover:bg-orange-700 text-foreground rounded-full h-11 w-11 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
              ...
              {questions.length > 0 && (
                <div className="mt-8 flex justify-center">
                  <Button variant="ghost" onClick={() => {
                    setQuestions([]);
                    sessionStorage.removeItem("theory_track");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSubmissions.map((sub, idx) => (
                <div 
                  ...
                  onClick={() => {
                    if (sub.problemData) sessionStorage.setItem("current_theory_problem", JSON.stringify(sub.problemData));
                    router.push("/theory/practice");
                  }}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-zinc-500/10 text-zinc-500 flex items-center justify-center shrink-0 border border-zinc-500/20">
                        <BookOpen className="w-5 h-5" />
                      </div>
                      <div>
                        <h3 className="font-bold text-foreground text-[15px] leading-tight mb-0.5 group-hover:text-zinc-500 transition-colors truncate max-w-[150px]">{sub.problemTitle || sub.title || "Theory Test"}</h3>
